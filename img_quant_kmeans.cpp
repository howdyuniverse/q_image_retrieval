...
public:
	...
	{
		// ќпредел€ем самый близкий кластер к данному пикселю
		word best_k = 0;
		double dist = 0;
		for (word i = 0; i < k_num; i++) {
	...
	{
		// ≈вклидово рассто€ние между 3 компонентами цвета
		double dist = 0;
...
void getImgKmeansHist(char * img_file)
{
	...
	{
		moves = 0;
		// проходимс€ по кластерам пересчитыва€ центры
		for (word clust = 0; clust < k; clust++) {
			//cout << "DEBUG> cluster iter" << endl;
			unsigned int pixel_num = 0; // число пикселей вход€щих в кластер
			unsigned int sum_comp1 = 0; // считаем сумму каждого компонента
			unsigned int sum_comp2 = 0; // дл€ вычислени€ сред.
			unsigned int sum_comp3 = 0;

			// проходимс€ по всем цветам и смотрим по таблице индексов
